...
Supabase CLI は `import_map` を参照するため、`supabase/functions/import-map.json` を用意し、
`supabase/config.toml` から参照する。`deno.json` は editor 向けの設定として残す。
...
各 function に `deno.json` を作る必要はない。
共通の `supabase/functions/deno.json` と `import-map.json` を使う運用。
ただし `supabase/config.toml` に以下を追加する:
...
CLI の Deno と `deno.lock` のバージョンが合わない場合はエラーになる。
その場合は `supabase/functions/deno.lock` を削除して再デプロイする。
